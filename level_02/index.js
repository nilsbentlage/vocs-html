...
const modusFeld = document.getElementById("modus");

// Initialisiere Variablen
let aktuelleVokabel = null;
let animationsTimer = null;
let frageModus = "deutsch";
let punktestand = 0;
...
function neueVokabelAnzeigen() {
  ...
  vokabelFeld.innerText = aktuelleVokabel[frageModus];
}
...
function vergleicheAntwort() {
  const antwortModus = frageModus === "deutsch" ? "englisch" : "deutsch";
  const antwort = antwortFeld.value;
  if (antwort.toLowerCase() === aktuelleVokabel[antwortModus].toLowerCase()) {
    zaehlePunkte();
    entferneVokabel();
...
function init() {
  ...
  modusFeld.addEventListener("change", (e) => {
    frageModus = e.target.value;
    antwortFeld.value = "";
    neueVokabelAnzeigen();
  });
  antwortFeld.focus();
